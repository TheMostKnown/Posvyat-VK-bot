        ...
        if event.type == VkEventType.MESSAGE_NEW and event.to_me:
            ...
            if text[:13] == "/get_members ":
                ...
                if is_admin(user_id, event):
                    ...
                    session = get_session(engine)
                    message = text.split()
                    groupps = ["/get_members", "first_group", "second_group", "third_group", "fourth_group", "fifth_group", "sixth_group", "seventh_group", "eighth_group", "ninth_group", "tenth_group", "eleventh_grup", "twelfth_group", "thirteenth_group"]
                    flag = 0
                    ...
                    if flag == 0:
                        ...
                        for c in q:
                            ...
                            if flag == 0:
                                ...
                                ind = 1
                                stroka = str(c.id) + " " + str(c.name) + " " + str(c.surname) + "\n" + str(c.vk_link) + "\n\n" + "Написал боту:" + str(c.first_group) + "\n\n" + "Вступил в группу:" + str(c.second_group) + "\n\n" + "Зарегистрировался:" + str(c.third_group) + "\n\n" + "Оплатил:" + str(c.fourth_group) + "\n\n" + "Отметил в форме трансфера - Парк победы:" + str(c.fifth_group) + "\n\n" + "Отметил в форме трансфера - Одинцово:" + str(c.sixth_group) + "\n\n" + "Отметил в форме трансфера - самостоятельно:" + str(c.seventh_group) + "\n\n" + "Прошел форму на расселение:" + str(c.eighth_group) + "\n\n" + "Согласовал ли трансфер:" + str(c.ninth_group) + "\n\n" + "Согласовал ли расселение:" + str(c.tenth_group) + "\n\n" + "Заказал мерч:" + str(c.eleventh_grup) + "\n\n" + "Подписался на бота в ТГ:" + str(c.twelfth_group) + "\n\n" + "Вернул ли билет:" + str(c.thirteenth_group)
                                send_message(vk_session, user_id, stroka)
            ...
            if text[:12] == "/give_level ":
                ...
                if is_admin(user_id, event):
                    ...
                    try:
                        ...
                        groupps = ["first_group", "second_group", "third_group", "fourth_group", "fifth_group", "sixth_group", "seventh_group", "eighth_group", "ninth_group", "tenth_group", "eleventh_grup", "twelfth_group", "thirteenth_group"]
            ...
            if text == "/commands":
                ...
                if is_admin(user_id, event):
                    ...
                    send_message(vk_session, user_id, "/start_mailing name first_group third_group ninth_group ... и так далее через пробел, где name - имя рассылки, а first_group, second_group... группы пользователей, в которых должно стоять значение True.\nНапример, чтобы запустить рассылку с именем 'test' для тех, кто оплатил билет и подписался на бота в тг, нужно ввести команду\n'/start_mailing test first_group twelfth_group', а для тех, кто просто оплатил билет,\n'/start_mailing test first_group'")
                    send_message(vk_session, user_id, "/give_level id level True/False, где level - название группы (first_group, second_group..., но только одна группа на одну команду!), id -  айди пользователя")
                    send_message(vk_session, user_id, "/get_members first_group third_group ninth_group ..., и так далее через пробел, где first_group, second_group... группы пользователей, в которых должно стоять значение True.")
                    ...
                    send_message(vk_session, user_id, "/start_mailing_all name, где name имя рассылки. Начинает всеобщую спам-атаку.")
